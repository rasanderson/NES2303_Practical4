...
However, we've just discussed continuous, whole number, proportion and binary data.
The last three break the assumptions of a linear model, and need to be analysed
using different error distributions. The four types of data (if we include continuous)
can be represented by three statistical distributions:
...
Friederick Gauss. The graph below shows a frequency histogram of 100 random numbers,
...
```{r gaussian_distribution, echo=TRUE, cache=TRUE, warning=FALSE}
mydata <- rnorm(n = 100, mean = 50, sd = 5)
gf_histogram(~mydata) %>% 
  gf_lims(x = c(0, 100))
```

 Look at the scale of the x-axis and it is obvious that we have continuous data with
...
The Poisson distribution for counts is named after Simeon Poisson (French
mathematician). Remember, this returns whole numbers (no decimals) that are 
greater than or equal to zero. It takes a single parameter $\lambda$ (Greek
letter lambda) which adjusts the shape of this distribution. Try the interactive
